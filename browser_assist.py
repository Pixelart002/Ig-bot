...
def cdp_call(ws,method,params=None,request_id=1,session_id=None):
    ...
    ws.send(json.dumps(message)); deadline=time.time()+30
    while time.time()<deadline:
        try:
            ...
            if "id" not in incoming:
                ...
                if event in {"Target.detachedFromTarget","Inspector.targetCrashed","Runtime.executionContextDestroyed"}: logging.warning("CDP ASYNC EVENT: %s",event)
                continue
            if incoming.get("id")==request_id: return incoming
        except Exception as exc:
            logging.error("WebSocket recv error during %s: %s",method,exc); raise RuntimeError(f"WebSocket failure during {method}: {exc}") from exc
...
def _prepare_page(tab):
    for index,method in enumerate(("Page.enable","Runtime.enable","Network.enable"),start=10):
        ...
        if result.get("error"): logging.warning("%s failed: %s",method,result["error"])
def _socket_alive(ws): return bool(ws is not None and getattr(ws,"connected",True))
def _evaluate(tab,expression):
    lock=tab.setdefault("_lock",threading.RLock())
    with lock:
        ws=tab.get("_ws"); session_id=tab.get("sessionId")
        if ws is None or not session_id: raise RuntimeError("Lightpanda CDP session is unavailable; original connection is gone")
        last_error=None
        for attempt in range(3):
            try:
                result=cdp_call(ws,"Runtime.evaluate",{"expression":expression,"returnByValue":True},100+attempt,session_id)
                if not result.get("error"): return result.get("result",{}).get("result",{}).get("value")
                err=result["error"]; message=str(err.get("message",""))
                last_error=RuntimeError(f"Lightpanda execution context unavailable: {message}" if ("BrowserContextNotLoaded" in message or "Cannot find context" in message) else f"CDP Runtime.evaluate failed: {err}")
            except Exception as exc: last_error=exc
            if attempt<2: time.sleep(0.5)
        if not _socket_alive(ws): raise RuntimeError("Lightpanda CDP connection closed; connection-scoped target cannot be reattached") from last_error
...
def start_keepalive(tab,interval=15.0):
    stop_event=tab.setdefault("_keepalive_stop",threading.Event()); existing=tab.get("_keepalive_thread")
    if existing and existing.is_alive(): return stop_event,existing
    ws=tab.get("_ws")
    if ws is None: logging.warning("Keepalive not started: original connection unavailable"); return stop_event,threading.current_thread()
    def loop():
        logging.info("Network keepalive started for WS connection")
        while not stop_event.is_set() and tab.get("_ws") is ws:
            try: ws.ping()
            except Exception as exc: logging.warning("Keepalive ping failed: %s",exc); break
            stop_event.wait(interval)
    thread=threading.Thread(target=loop,name="ws-keepalive",daemon=True); tab["_keepalive_thread"]=thread; thread.start(); return stop_event,thread
...
def open_url(url):
    ...
    for attempt in range(3):
        ws=None
        try:
            cdp_version(); ws=_create_browser_connection()
            # Create the target at the final URL. Do not Page.navigate afterward:
            # Lightpanda has known CDP navigation/context lifecycle edge cases.
            created=cdp_call(ws,"Target.createTarget",{"url":url},request_id=1)
            if created.get("error"): raise RuntimeError(f"CDP Target.createTarget failed: {created['error']}")
            target_id=created.get("result",{}).get("targetId")
            if not target_id: raise RuntimeError("CDP Target.createTarget returned no targetId")
            attached=cdp_call(ws,"Target.attachToTarget",{"targetId":target_id,"flatten":True},request_id=2)
            if attached.get("error"): raise RuntimeError(f"CDP Target.attachToTarget failed: {attached['error']}")
            session_id=attached.get("result",{}).get("sessionId")
            if not session_id: raise RuntimeError("CDP Target.attachToTarget returned no sessionId")
            ...
            _prepare_page(tab); return tab
        except WebSocketBadStatusException as exc: last_error=RuntimeError(f"Lightpanda CDP websocket handshake failed: {exc}")
        except (requests.RequestException,OSError,ValueError,RuntimeError,TimeoutError) as exc: last_error=exc
        finally:
            if ws is not None and last_error is not None:
                try: ws.close()
                except Exception: pass
        if attempt<2: logging.warning("Open tab attempt %d/3 failed: %s",attempt+1,last_error); time.sleep(1)
    raise last_error or RuntimeError("Unable to open Lightpanda tab")
def open_signup(): return open_url(SIGNUP_URL)
def close_tab(tab):
    stop_keepalive(tab); ws=tab.get("_ws")
    if ws is not None:
        try: ws.close()
        except Exception: pass
...
def _username_available(tab,username):
    ...
    if not result or result.get("state")!="entered": return False
    ...
    for _ in range(8):
        ...
        if unavailable.search(text) or str(state.get("invalid","")).lower()=="true":
            logging.info("Username rejected by Instagram: %s",username); return False
...
def select_available_username(tab,identity):
    ...
    for value in identity.get("usernames") or []:
        ...
        if not candidate or candidate.lower() in seen: continue
        seen.add(candidate.lower())
        try:
            if _username_available(tab,candidate):
                identity["selected_username"]=candidate; identity["username"]=candidate; return candidate
        except Exception as exc:
            logging.warning("Username check failed for %s: %s",candidate,exc)
    return None
def fill_fields(tab,credentials=None,identity=None):
    credentials=credentials or {}; identity=identity or {}
    email=credentials.get("email") or os.getenv("IG_EMAIL"); password=credentials.get("password") or os.getenv("IG_PASSWORD"); username=str(identity.get("selected_username") or credentials.get("username") or "").strip(); full_name=credentials.get("full_name",""); dob=identity.get("date_of_birth") or credentials.get("date_of_birth")
    if not email or not password or not username: logging.error("Email, password and confirmed username are required."); return False
    ...
    for attempt in range(15):
        try:
            result=_evaluate(tab,f"({expression})({json.dumps(values)})"); logging.info("Signup fields attempt %d: %s",attempt+1,result)
            if result and result.get("email") and result.get("password") and result.get("username"): return True
        except Exception as exc: logging.warning("Signup fields evaluation error on attempt %d: %s",attempt+1,exc)
        time.sleep(1)
    return False
def submit_signup(tab): return bool(_evaluate(tab,"""()=>{const b=[...document.querySelectorAll('button')].find(x=>/sign up|create account/i.test((x.innerText||'').trim()));if(!b||b.disabled)return false;b.click();return true;}"""))
...
def inspect_state(tab):
    value=_evaluate(tab,"JSON.stringify({url:location.href,title:document.title,text:(document.body?.innerText||'').slice(0,4000)})"); data=json.loads(value or "{}"); text=data.get("text","").lower()
    if any(x in text for x in ("security code","confirmation code","enter the code","confirm your email")): status="otp_required"
    elif "captcha" in text: status="captcha_required"
    elif "welcome to instagram" in text: status="completed"
    else: status="waiting"
...
def main():
    try:
        version=cdp_version(); credentials={"email":os.getenv("IG_EMAIL",""),"password":os.getenv("IG_PASSWORD",""),"username":os.getenv("IG_USERNAME",""),"full_name":os.getenv("IG_FULL_NAME","")}; tab,filled=start_signup(credentials)
        try: print(f"Connected: {version.get('Browser','unknown browser')}"); print(f"Signup tab: {tab.get('url',SIGNUP_URL)}"); print(f"Form fields filled: {filled}"); print("CAPTCHA/OTP/verification remain manual."); return 0
        finally: close_tab(tab)
    except Exception as exc: logging.exception("Browser assistant failed: %s",exc); return 1
