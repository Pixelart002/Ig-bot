...
def _handle_message(message: dict) -> None:
    ...
    if text.isdigit() and len(text) == 6:
        # Accept a direct OTP even if the status update arrived slightly late.
        if queue_otp_code(text):
            session.event("otp_received_from_telegram", "success")
            return
...
def run_polling() -> None:
    """Run Telegram getUpdates directly; no webhook or public bridge required."""
    if not bot.TOKEN:
        raise SystemExit("Set TELEGRAM_BOT_TOKEN")

    # Remove any old webhook so getUpdates cannot fail with a 409 conflict.
    try:
